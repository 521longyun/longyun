...
common.main = {
	        ...
	        $(document).on("click","#releaseDownloadPDFBtn:not(.wbd-vip-lock)",function(){
	    		...
	    		if(downloadFlag){
	    			...
	    			if(param!=""&&visitid!=""&&visitid!=undefined){
	    			   $.get("/cvresume/resume_email_track/"+location.search+"&replyType=hrDownload");
	    			}
	    		}
	    	});
	    },
	    ...
	    resume_confirm:function(options){//系统确认性弹框
			...
			'				<button type="button" class="close 500dtongji" data_track="PC-MB3.0.1-通用-通用-通用-右上角-关闭" data-dismiss="modal" aria-hidden="true"></button>'+
			'			</div>'+
...
//            common.main.text_maxlength($("#search_btn"),200); 
        ...
        send_resume:function(){
            ...
            $send_resume_modal.html('\n' +
                ...
                '                        </div>\n' +
                '                    </div>\n' +
                '                    <div class="preview_button_bar"><a href="javascript:;" class="cancel_button">返回</a><a href="javascript:;" class="ok_button">确认投递</a></div>\n' +
                '                </div>\n' +
                '            </div>\n' +
                '        </div>');
